...
const contributions = [
  { id: 1, name: "Nicholus Jiveri", registration: 500, jan: 500, feb: 500, mar: 500, janPaidDate: 6, febPaidDate: 7 },
  { id: 2, name: "Durusila Jiveri", registration: 500, jan: 500, feb: 500, mar: 500, janPaidDate: 4 }, 
  // ... add janPaidDate (or any month) for each member
];

// Extract months dynamically (excluding id & name & paid date fields)
const months = Object.keys(contributions[0]).filter(
  (key) => key !== "id" && key !== "name" && !key.toLowerCase().includes("paiddate")
);

// Fine per late payment
const LATE_FINE = 100;

// Calculate totals per month including fines
const totals = {};
months.forEach((month) => {
  totals[month] = contributions.reduce((sum, c) => {
    let value = c[month] || 0;
    // Check if there is a paid date for the month
    const paidDateKey = month + "PaidDate";
    if (c[paidDateKey] && c[paidDateKey] < 5) {
      value += LATE_FINE; // add fine if paid before 5th
    }
    return sum + value;
  }, 0);
});
...
//   return (
//     <div className="max-w-7xl mx-auto mt-10 bg-white shadow-lg rounded-lg p-6">
//       <h2 className="text-2xl font-bold mb-6 text-center">Vamuli Welfare Contributions(Year 2026)</h2>
