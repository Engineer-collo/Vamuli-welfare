...
months.forEach((month) => {
  totals[month] = contributionsData.reduce((sum, c) => {
    ...
    let value = c[month] || 0;
    // Apply fine if payment is late (after 5th of the following month)
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {contributionsWithTotals.map((c) => {
            // Highlight row if any late payment
            const hasFine = months.some((month) => {
              const paidDateKey = month + "PaidDate";
              return c[paidDateKey] && c[paidDateKey] > 5;
            });
