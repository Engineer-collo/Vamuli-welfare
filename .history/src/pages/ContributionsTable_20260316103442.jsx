...
const contributions = [
  ...
  { id: 2, name: "Durusila Jiveri", registration: 500, jan: 500, feb: 0, mar: 500 },
  { id: 3, name: "Janet Jiveri", registration: 500, jan: 500, feb: 500, mar: 0 },
...
// Dynamically extract months (exclude id & name)
...
// Grand total
const grandTotal = contributions.reduce(
  (sum, c) => sum + months.reduce((memberSum, month) => memberSum + c[month], 0),
  0
);
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 bg-white shadow-lg rounded-lg p-6">
      ...
      <div className="flex flex-wrap justify-center gap-6 mb-6 text-lg font-semibold">
        ...
        <span className="text-green-800">Grand Total: Ksh {grandTotal} ✅</span>
      </div>
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {contributions.map((c) => {
            const memberTotal = months.reduce((sum, month) => sum + c[month], 0);
            return (
              <tr key={c.id} className="border-b hover:bg-gray-100 transition">
                ...
                <td className="p-3 font-bold">{memberTotal}</td>
              </tr>
            );
          })}
        </tbody>
