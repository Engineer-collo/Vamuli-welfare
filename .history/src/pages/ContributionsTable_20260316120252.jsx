...
// Extract months dynamically (exclude id, name, role, and PaidDate fields)
...
months.forEach((month) => {
  totals[month] = contributionsData.reduce((sum, c) => {
    ...
    // Add fine only for monthly contributions, registration is never fined
    if (month !== "registration" && c[paidDateKey] && c[paidDateKey] > 5) {
      value += LATE_FINE;
    }
...
const contributionsWithTotals = contributionsData.map((c) => {
  const total = months.reduce((sum, month) => {
    ...
    // Apply fine only for monthly contributions
    if (month !== "registration" && c[paidDateKey] && c[paidDateKey] > 5) {
      value += LATE_FINE;
    }
...
// Calculate full total for all members
          ...
          {contributionsWithTotals.map((c) => (
            <tr key={c.id} className="border-b hover:bg-gray-100 transition">
              ...
              {months.map((month) => {
                ...
                // Apply fine only for monthly contributions
                const late = month !== "registration" && c[paidDateKey] && c[paidDateKey] > 5;
